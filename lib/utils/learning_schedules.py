...
def exponential_decay_with_burnin(global_step,
	...
	post_burnin_learning_rate = tf.train.exponential_decay(
		...
		staircase=True)
	return tf.where(
		condition=tf.less(tf.cast(global_step, tf.int32), tf.constant(burnin_steps)),
		x=tf.constant(burnin_learning_rate),
		y=post_burnin_learning_rate, 
		name='learning_rate')
...
def cosine_decay_with_warmup(global_step,
	...
	if hold_base_rate_steps > 0:
		learning_rate = tf.where(condition=global_step > warmup_steps + hold_base_rate_steps,
		             			 x=learning_rate, y=learning_rate_base)
	if warmup_steps > 0:
		slope = (learning_rate_base - warmup_learning_rate) / warmup_steps
		warmup_rate = slope * tf.cast(global_step,
	                  				  tf.float32) + warmup_learning_rate
		learning_rate = tf.where(condition=global_step < warmup_steps, 
								 x=warmup_rate, y=learning_rate)
